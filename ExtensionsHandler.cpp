...
namespace GLTF
{
	namespace
	{
		static const TArray<FString> LightExtensions = {TEXT("KHR_lights_punctual"), TEXT("KHR_lights"), TEXT("KHR_lights_area"), TEXT("KHR_lights_environment")};
		static const TArray<FString> MaterialsExtensions = {TEXT("KHR_materials_variants")};

		TMap<FString, TSharedPtr<FJsonObject>>  GetLightExtensions(const TSharedPtr<FJsonObject>& Object)
		{
			// TArray<TSharedPtr<FJsonObject>> LightsObjs;

			TMap<FString, TSharedPtr<FJsonObject>> LightsObjs;

			for(FString LightExt : LightExtensions) {
				if (Object->HasTypedField<EJson::Object>(LightExt)) {
					// LightsObjs.Emplace(Object->GetObjectField(LightExt)); 
					LightsObjs.Emplace(LightExt, Object->GetObjectField(LightExt));
				}
			}
		...
		TSharedPtr<FJsonObject> GetLightExtension(const TSharedPtr<FJsonObject>& Object)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("Light type"));
			...
			} else if (Object->HasTypedField<EJson::Object>(LightExtensions[2]))
			{
				// TODO @mswamy : Allow punctual and area lights at the same time.
				LightsObj = Object->GetObjectField(LightExtensions[2]);
			} else if (Object->HasTypedField<EJson::Object>(LightExtensions[3]))
			{
				LightsObj = Object->GetObjectField(LightExtensions[3]);
			}
			return LightsObj;
		}
	...
	void FExtensionsHandler::SetupAssetExtensions(const FJsonObject& Object) const
	{
		...
		// TArray<TSharedPtr<FJsonObject>> LightsObjs = GetLightExtensions(ExtensionsObj);

		TMap<FString, TSharedPtr<FJsonObject>> LightsObjs = GetLightExtensions(ExtensionsObj);
		if (LightsObjs.Num() > 0) 
		{
			if (ExtensionsObj->HasTypedField<EJson::Object>(LightExtensions[0])) {
				Asset->ExtensionsUsed.Add(EExtension::KHR_LightsPunctual);
			}

			// Asset->ExtensionsUsed.Add(EExtension::KHR_LightsPunctual);
			if (ExtensionsObj->HasTypedField<EJson::Object>(LightExtensions[2])) {
				Asset->ExtensionsUsed.Add(EExtension::KHR_LightsArea);
			}

			if (ExtensionsObj->HasTypedField<EJson::Object>(LightExtensions[3])) {
				Asset->ExtensionsUsed.Add(EExtension::KHR_LightsEnvironment);
			}

			// for (TSharedPtr<FJsonObject> LightsObj : LightsObjs) 
			for (auto& Elem : LightsObjs) 
			{
				TSharedPtr<FJsonObject> LightsObj = Elem.Value;
				FString LightExt = Elem.Key;
				...
				if (LightCount > 0)
				{
					...
					for (const TSharedPtr<FJsonValue>& Value : LightsObj->GetArrayField(TEXT("lights")))
					{
						...
						// if (ExtensionsObj->HasTypedField<EJson::Object>(LightExtensions[2])) {
						// 	SetupLightArea(*Value->AsObject());
						// }

						// if (ExtensionsObj->HasTypedField<EJson::Object>(LightExtensions[3])) {
						// 	SetupLightEnvironment(*Value->AsObject());
						// }

						if (LightExt.Compare(LightExtensions[0]) == 0 ) {
							SetupLightPunctual(*Value->AsObject());
						}
						
						if (LightExt.Compare(LightExtensions[2]) == 0) {
							SetupLightArea(*Value->AsObject());
						}

						if (LightExt.Compare(LightExtensions[3]) == 0) {
							SetupLightEnvironment(*Value->AsObject());
						}
					}
				}
	...
	void FExtensionsHandler::SetupLightArea(const FJsonObject& Object) const
	{
		const uint32 LightIndex = Asset->Lights.Num();
		const FNode* Found      = Asset->Nodes.FindByPredicate([LightIndex](const FNode& Node) { return LightIndex == Node.LightIndex; });
		...
		const FString Type = GetString(Object, TEXT("shape"));
		if (Type == TEXT("rect"))
		{
			Light.Type = FLight::EType::RectArea;
			Light.Width      = GetScalar(Object, TEXT("width"), 1.f);
			Light.Height      = GetScalar(Object, TEXT("height"), 1.f);
		}
		else
			Messages.Emplace(EMessageSeverity::Warning, FString::Printf(TEXT("Light has no type specified: %s"), *Light.Name));
	}

	void FExtensionsHandler::SetupLightEnvironment(const FJsonObject& Object) const
	{
		const uint32 LightIndex = Asset->Lights.Num();
		const FNode* Found      = Asset->Nodes.FindByPredicate([LightIndex](const FNode& Node) { return LightIndex == Node.LightIndex; });
		...
		Light.Intensity = GetScalar(Object, TEXT("intensity"), 1.f);
		Light.Hdri = GetString(Object, TEXT("hdri"));
		Light.Type = FLight::EType::EnvMap;
	}
